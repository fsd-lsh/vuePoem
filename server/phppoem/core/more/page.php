...
class page {
    ...
    static function run($m, $url = '', $page_size = 15, $show_nums = 5) {
        ...
        $info['html']       = self::pagehtml2($page, $info['page_count'], $info['url'], $show_nums, $total);
        return $info;
    }
     ...
     * Desc: ElementUI PAGE
     */
    static function pagehtml2($np, $tp, $url, $num = 5, $total) {
        $up   = $np - 1; // 上一页
        $dp   = $np + 1; // 下一页
        $f    = ($np == 1) ? false : true; // 是否为首页
        $e    = ($np == $tp) ? false : true; // 是否问尾页
        $html = '';
        if ($tp > 0) {
            $html .= '<div class="el-pagination is-background">';
            $html .= '<el-tag size="mini">共 ' . $total . ' 条 ' . $np . '/' . $tp . '页</el-tag>';
            if ($np != 1 && $f) {
                $html .= '<button type="button" class="btn-prev"><a href="' . self::geturl($url, 1) . '"> << </a></button>';
                $html .= '<button type="button" class="btn-prev"><a href="' . self::geturl($url, $up) . '"> < </a></button>';
            }
            ...
            $html .= '</ul>';
            if ($np != $tp && $e) {
                $html .= '<button type="button" class="btn-next"><a href="' . self::geturl($url, $dp) . '"> > </a></button>';
                $html .= '<button type="button" class="btn-next"><a href="' . self::geturl($url, $tp) . '"> >> </a></button>';
            }
            $html .= "</div>";
        }
        return $html;
